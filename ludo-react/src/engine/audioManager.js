// LUDO NEON - Audio Manager
// Ported directly from audio.js — unchanged logic, exported as singleton
...
class LudoAudioManager {
  ...
  toggleMute() {
    ...
    if (this.musicGainNode) this.musicGainNode.gain.setValueAtTime(this.muted ? 0 : this.musicVolume, this.ctx.currentTime);
    if (this.sfxGainNode) this.sfxGainNode.gain.setValueAtTime(this.muted ? 0 : this.sfxVolume, this.ctx.currentTime);
    return this.muted;
  }

  setMusicVolume(vol) {
    this.musicVolume = vol;
    if (this.musicGainNode) this.musicGainNode.gain.setTargetAtTime(this.muted ? 0 : vol, this.ctx.currentTime, 0.05);
  }

  setSfxVolume(vol) {
    this.sfxVolume = vol;
    if (this.sfxGainNode) this.sfxGainNode.gain.setTargetAtTime(this.muted ? 0 : vol, this.ctx.currentTime, 0.05);
  }
  ...
  playStep(stepIndex = 0) {
    ...
    if (sound) sound.osc.frequency.exponentialRampToValueAtTime(targetFreq * 1.5, this.ctx.currentTime + duration);
  }
  ...
  playCapture() {
    ...
    gainNode.connect(this.sfxGainNode || this.ctx.destination);
    osc.start(now); oscNoise.start(now);
    osc.stop(now + duration); oscNoise.stop(now + duration);
  }
  ...
  playHome() {
    ...
    const now = this.ctx.currentTime;
    const chords = [
      { f: [261.63, 329.63, 392.00], t: 0 },
      { f: [392.00, 493.88, 587.33], t: 0.15 },
      { f: [523.25, 659.25, 783.99, 1046.50], t: 0.3 }
    ];
  ...
  playWin() {
    ...
    scale.forEach((freq, idx) => {
      ...
      osc.connect(gainNode);
      gainNode.connect(this.sfxGainNode || this.ctx.destination);
      vibrato.start(time); osc.start(time);
      vibrato.stop(time + duration); osc.stop(time + duration);
    });
  }
  ...
  playSynthwaveLoop() {
    ...
    const playBassTone = (freq, duration) => {
      ...
      try {
        ...
        osc.start();
        osc.stop(this.ctx.currentTime + duration);
      } catch (e) { console.error('Synth play error:', e); }
    };
    ...
    this.bassInterval = setInterval(() => {
      ...
      if (step % 8 === 0) {
        if (step % 16 === 0) playPadTone([220, 261.63, 329.63], 3.0);
        else playPadTone([174.61, 220, 261.63], 3.0);
      }
      step++;
    }, 400);
  }
...
export const audioManager = new LudoAudioManager();
